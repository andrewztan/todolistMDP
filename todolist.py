...
if __name__ == '__main__':
    ...
    biggestTimeCost = max([task.getTimeCost() for task in my_list.getTasks()])
    V_states = {}
    for t in range(end_time+biggestTimeCost):
    ...
    def sumTransitionStates(mdp, state, action, trans_states_and_probs, V_states):
        total = 0
    ...
    while not converged:
        ...
        for state in V_states:
            ...
            for a in possible_actions:
                ...
                value = sumTransitionStates(mdp, state, a, trans_states_and_probs, V_states)
                if value > best_value:
                    best_value = value
                    best_action = a
